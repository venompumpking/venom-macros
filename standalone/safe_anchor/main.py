"""Zenith Macros — Safe Anchor (standalone CLI)"""
import sys, os
...
BANNER  = f"""
{Fore.MAGENTA}╔══════════════════════════════════════════╗
║   Zenith Macros — Safe Anchor    v{VERSION}   ║
╚══════════════════════════════════════════╝{Style.RESET_ALL}"""
...
def do_verify(cfg, force=False):
    ...
    if result["ok"]:
        ...
        print_ok(f"Entitlement confirmed — {result.get('product_name','Safe Anchor')} is yours")
        ...
        if code == "no_entitlement": print_info("Buy Safe Anchor at: zenithmacros.store")
...
def setup_settings(cfg):
    ...
    print(); print_info("Actions: which steps fire when you press the keybind.")
    cfg["actions"] = []
    for action in ["place","charge","explode"]:
        cur = action in (cfg.get("actions") or ["place","charge","explode"])
        if confirm(f"  Enable {action.upper()}?", default=cur): cfg["actions"].append(action)
    if not cfg["actions"]: cfg["actions"] = ["place","charge","explode"]; print_info("Defaulting to all three.")
...
def main_menu(cfg):
    while True:
        ...
        print(f"  Keybind: {Fore.YELLOW}[{kb.upper()}]{Style.RESET_ALL}  |  Delay: {cfg.get('delay_ms',27)}ms  |  Actions: {', '.join(cfg.get('actions',[])).upper()}")
