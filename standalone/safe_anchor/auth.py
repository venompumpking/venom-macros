...
PRODUCT_ID = "zenith-safe-anchor"
HEADERS    = {"Content-Type": "application/json", "User-Agent": "ZenithSafeAnchor/1.0"}
...
def verify_license(raw_key):
    ...
    if not er.get("granted"): return {"ok": False, "error": "This key does not own Safe Anchor. Purchase it at zenithmacros.store", "code": "no_entitlement"}
    return {"ok": True, "session_token": session_token, "tier": vr.get("tier", ""), "product_name": er.get("product_name", "Safe Anchor")}
