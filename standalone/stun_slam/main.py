"""Zenith Macros — Stun Slam (standalone CLI)"""
import sys, os
...
BANNER  = f"""
{Fore.MAGENTA}╔══════════════════════════════════════════╗
║   Zenith Macros — Stun Slam      v{VERSION}   ║
╚══════════════════════════════════════════╝{Style.RESET_ALL}"""
...
def do_verify(cfg, force=False):
    ...
    if result["ok"]:
        ...
        print_ok(f"Entitlement confirmed — {result.get('product_name','Stun Slam')} is yours")
        ...
        if code == "no_entitlement": print_info("Buy Stun Slam at: zenithmacros.store")
...
def setup_settings(cfg):
    ...
    cfg["mace_key"] = prompt("Mace slot key", cfg.get("mace_key", "5"))
    cfg["delay_ms"] = int(prompt("Delay between axe/mace (ms)", str(cfg.get("delay_ms", 27))) or 27)
...
def show_config(cfg):
    ...
    print(f"  {'Mace key':<22} {cfg.get('mace_key','5')}")
...
def main_menu(cfg):
    while True:
        ...
        print(f"  Keybind: {Fore.YELLOW}[{kb.upper()}]{Style.RESET_ALL}  |  Axe: {cfg.get('axe_key','4')}  |  Mace: {cfg.get('mace_key','5')}  |  Delay: {cfg.get('delay_ms',27)}ms")
