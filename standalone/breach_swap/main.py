"""Zenith Macros — Breach Swap (standalone CLI)"""
import sys, os
...
BANNER  = f"""
{Fore.MAGENTA}╔══════════════════════════════════════════╗
║   Zenith Macros — Breach Swap    v{VERSION}   ║
╚══════════════════════════════════════════╝{Style.RESET_ALL}"""
...
def do_verify(cfg, force=False):
    ...
    if result["ok"]:
        ...
        print_ok(f"Entitlement confirmed — {result.get('product_name','Breach Swap')} is yours")
        ...
        if code == "no_entitlement": print_info("Buy Breach Swap at: zenithmacros.store")
...
def setup_settings(cfg):
    ...
    cfg["mace_key"]  = prompt("Mace slot key",  cfg.get("mace_key",  "4"))
    cfg["sword_key"] = prompt("Sword slot key", cfg.get("sword_key", "5"))
    cfg["delay_ms"]  = int(prompt("Delay between mace/sword (ms)", str(cfg.get("delay_ms", 27))) or 27)
...
def show_config(cfg):
    ...
    print(f"  {'Mace key':<22} {cfg.get('mace_key','4')}")
...
def main_menu(cfg):
    while True:
        ...
        print(f"  Keybind: {Fore.YELLOW}[{kb.upper()}]{Style.RESET_ALL}  |  Mace: {cfg.get('mace_key','4')}  |  Sword: {cfg.get('sword_key','5')}  |  Delay: {cfg.get('delay_ms',27)}ms")
