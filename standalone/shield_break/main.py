"""Zenith Macros — Shield Break (standalone CLI)"""
import sys, os
...
BANNER  = f"""
{Fore.MAGENTA}╔══════════════════════════════════════════╗
║   Zenith Macros — Shield Break   v{VERSION}   ║
╚══════════════════════════════════════════╝{Style.RESET_ALL}"""
...
def do_verify(cfg, force=False):
    ...
    if result["ok"]:
        ...
        print_ok(f"Entitlement confirmed — {result.get('product_name','Shield Break')} is yours")
        ...
        if code == "no_entitlement": print_info("Buy Shield Break at: zenithmacros.store")
...
def setup_settings(cfg):
    ...
    cfg["axe_key"]        = prompt("Axe slot key",            cfg.get("axe_key",         "4"))
    cfg["sword_key"]      = prompt("Sword slot key",          cfg.get("sword_key",        "5"))
    cfg["double_click_ms"] = int(prompt("Double-click gap (ms)", str(cfg.get("double_click_ms", 2))) or 2)
...
def show_config(cfg):
    ...
    print(f"  {'Axe key':<22} {cfg.get('axe_key','4')}")
    print(f"  {'Sword key':<22} {cfg.get('sword_key','5')}")
    print(f"  {'Double-click gap (ms)':<22} {cfg.get('double_click_ms',2)}")
    sep()
...
def main_menu(cfg):
    while True:
        ...
        print(f"  Keybind: {Fore.YELLOW}[{kb.upper()}]{Style.RESET_ALL}  |  Axe: {cfg.get('axe_key','4')}  |  Sword: {cfg.get('sword_key','5')}")
