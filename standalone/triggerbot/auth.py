...
PRODUCT_ID = "zenith-triggerbot"
HEADERS    = {"Content-Type": "application/json", "User-Agent": "ZenithTriggerbot/1.0"}
...
def verify_license(raw_key):
    ...
    if not er.get("granted"): return {"ok": False, "error": "This key does not own Triggerbot. Purchase it at zenithmacros.store", "code": "no_entitlement"}
    return {"ok": True, "session_token": session_token, "tier": vr.get("tier", ""), "product_name": er.get("product_name", "Triggerbot")}
